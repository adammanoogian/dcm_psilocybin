...
class PEBPlotter:
    ...
    def plot_heatmaps(self):
        ...
        for i_cov in range(cov_n):
            ...
            # Viridis colormap for behavioral associations
            elif peb_type == 'behav_associations':
                cmap = 'viridis'
            ...
            if cov_names and len(cov_names) > i_cov:
                ...
                # Use short title if available, otherwise use covariate name
    ...
    def save_all_pngs(self, output_dir=None, dpi=300):
        """Save all generated figures as PNG files for easy combination."""
        if not self.figures:
            raise RuntimeError("No figures to save. Run plot_heatmaps() first.")
        base_name = os.path.splitext(os.path.basename(self.mat_file_path))[0]
        if output_dir is None:
        ...
        for fig, covariate_name in zip(self.figures, self.covariate_names):
            if covariate_name:
                safe_cov = str(covariate_name).replace(' ', '_').replace('/', '_')
                file_name = f"{base_name}_{safe_cov}.png"
            else:
                file_name = f"{base_name}.png"
            output_path = os.path.join(output_dir, file_name)
            fig.savefig(output_path, format='png', dpi=dpi, bbox_inches='tight', pad_inches=0.02)
        ...
        for i, mat_path in enumerate(mat_files, 1):
            ...
            try:
                ...
                if save:
                    ...
                    plotter.save_all_pngs(output_dir=output_dir, dpi=300)
                if show:
                    plotter.show_all()
