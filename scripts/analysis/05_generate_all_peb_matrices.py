...
def generate_peb_matrices(visualizer=None):
    """Generate PEB matrix heatmaps for all conditions."""

    print("=" * 70)
    print("COMPREHENSIVE PEB MATRIX HEATMAP GENERATION")
    print("=" * 70)
        ...
        try:
            ...
            if info['peb_type'] == 'change':
                ...
                if len(plotter.figures) > 1:
                    ...
                    print(f"  ✓ Saved: {output_file.name} (Covariate 1: Behavioral Association)")
                    generated_files.append(output_file)
                    stats['generated'] += 1
                    ...
                    print(f"  ✓ Saved: {output_file.name} (Only one covariate)")
                    generated_files.append(output_file)
                    stats['generated'] += 1
